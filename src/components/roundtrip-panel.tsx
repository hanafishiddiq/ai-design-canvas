...
export function RoundtripPanel() {
  ...
  const downloadCaptureHelper = () => {
    ...
    const source = `// Run this in generated/instrumented code that preserves data-page-id and data-node-id.\nconst PAGE_ID = ${JSON.stringify(activePage.id)};\nconst root = document.querySelector('[data-page-id="' + CSS.escape(PAGE_ID) + '"]');\nif (!root) throw new Error('Mapped page root not found: ' + PAGE_ID);\nconst rr = root.getBoundingClientRect();\nconst elements = [...root.querySelectorAll('[data-node-id]')];\nconst manifest = {\n  pageId: PAGE_ID, route: location.pathname, viewport: { width: innerWidth, height: innerHeight }, capturedAt: new Date().toISOString(),\n  nodes: elements.map((el) => { const r=el.getBoundingClientRect(), s=getComputedStyle(el); return { nodeId:el.dataset.nodeId, x:Math.round(r.x-rr.x), y:Math.round(r.y-rr.y), width:Math.round(r.width), height:Math.round(r.height), text:el.textContent?.trim(), color:s.color, background:s.backgroundColor, fontSize:parseFloat(s.fontSize), fontWeight:parseFloat(s.fontWeight), radius:parseFloat(s.borderRadius), visible:s.display!=='none'&&s.visibility!=='hidden'&&Number(s.opacity)>0 }; }).filter(n=>n.nodeId)\n};\ncopy(JSON.stringify(manifest,null,2));\nconsole.log(manifest);`;
