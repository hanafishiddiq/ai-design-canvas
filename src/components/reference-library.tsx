  ...
  return <div className="fixed inset-0 z-[83] grid place-items-center bg-black/65 p-6 backdrop-blur-sm">
    <div className="flex h-[min(760px,calc(100vh-3rem))] w-[min(1050px,calc(100vw-3rem))] flex-col overflow-hidden rounded-xl border border-[#343a45] bg-[#0d1015] shadow-2xl">
      <div className="flex h-12 shrink-0 items-center gap-2 border-b border-[#292e36] px-3">
        <ImagePlus size={14} className="text-[#8d99ff]" /><div className="flex-1"><div className="text-[12px] font-semibold">Reference library</div><div className="text-[9px] text-[#6e7784]">Screenshot, sketch and moodboard inputs stored with the project.</div></div>
      ...
      <div className="min-h-0 flex-1 overflow-auto p-4">
        {!project ? <div className="grid h-full place-items-center text-[11px] text-[#6d7582]">No project is saved yet.</div> : project.references.length === 0 ? <div className="grid h-full place-items-center"><div className="max-w-sm text-center"><ImagePlus className="mx-auto text-[#4f5865]" size={28} /><div className="mt-3 text-[12px] font-semibold">No visual references yet</div><div className="mt-1 text-[10px] leading-4 text-[#6f7885]">Upload a screenshot, hand-drawn wireframe, moodboard or asset. AI Design Canvas compresses and analyzes it locally before storing it in the portable project.</div></div></div> : <div className="grid grid-cols-2 gap-4 xl:grid-cols-3">{project.references.map((reference) => <article key={reference.id} className="overflow-hidden rounded-xl border border-[#2b3039] bg-[#111419]">
